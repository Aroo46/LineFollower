...
void USART_Transmit ( char data ) {
	/*czekaj aø bufor nadawczy bÍdzie pusty*/
	while(!(UCSRA & (1<<UDRE)) );
...
//Definicja Funkcji wysy≥ajπcej ca≥e stringi
void USART_puts ( char * s ) {
	while( *s ) USART_Transmit( *s++ );
}

//Definicja funkcji wysy≥ajπcej ≥aÒcuchy z pamiÍci FLASH
void USART_puts_P(const char *s) {
	register char c;
	while ((c = pgm_read_byte( s++) )) USART_Transmit(c);
}
